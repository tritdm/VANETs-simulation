...
{
    while (vehicleInsertQueue.size())
    {
        int routeId = intrand(vehicleInsertQueue.size());
        std::map<int, std::queue<std::string>>::iterator i = vehicleInsertQueue.begin();
        std::advance(i, routeId);
        ...
        else vehicleInsertQueue.erase(i);
    }
}
